...
class BBox(object):
    ...
    def set_smiles(self, smiles, symbols, molfile=None):          #pred['oringinal_coords'],pred['original_symbols'],pred['orignal_edges']
    ...
    if molscribe:
        ...
        if len(bbox_images) > 0:
            ...
            for (i, j), pred in zip(bbox_indices, predictions):
                ...
                original_symbols = pred.get('original_symbols', None)
                pred_reactions[i].bboxes[j].set_smiles(pred['smiles'],original_symbols)
                # pred_reactions[i].bboxes[j].set_smiles(pred['smiles'],pred['original_symbols'])
    if ocr:
    ...
    if molscribe:
        ...
        if len(bbox_images) > 0:
            ...
            for i, pred in zip(bbox_indices, predictions):
                ...
                deduplicated[i].bboxes[j].set_smiles(pred['smiles'],pred['original_symbols'])
...
def postprocess_coref_results(bboxes, image, molscribe = None, ocr = None, batch_size = 32):
    ...
    if molscribe:
        ...
        if len(bbox_images) > 0:
            ...
            for i, pred in zip(bbox_indices, predictions):
                #bbox_objects[i].set_smiles(pred['smiles'],pred['oringinal_coords'],pred['original_symbols'],pred['orignal_edges'])
                bbox_objects[i].set_smiles(pred['smiles'],pred['original_symbols'])
